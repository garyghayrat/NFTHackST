import { nftHackData } from '../backEnd/scripts/nftHackData.js'
import Contract from '../backEnd/abis/NFTHack.json'
...
export const update = async (dispatch) => {
  try{
    ...
    await loadNFTHackData(dispatch, contract)
    await loadNFTHackState(dispatch, contract)
    if(account && contract){
...
export const loadNFTHackData = async (dispatch, contract) => {
  try{
    ...
    fetch(uri)
      .then(res => res.json())
      .then(result => {
        if(result.image===nftHackData[0].image || Number(totalSupply)===nftHackData.length){
          dispatch(metadataLoaded(nftHackData))
        }
      });
      console.log(nftHackData)
  } catch (e) {
...
export const loadNFTHackState = async (dispatch, contract) => {
  try{
    const tab = []
    const totalSupply = await contract.methods.totalSupply().call()
    ...
    dispatch(nftHackStateLoaded(tab))
  } catch (e) {
    console.log('Error, load NFTHack state', e)
  }
}

export const buyNFTHack = async (dispatch, id, price) => {
  try{
    ...
    await contract.methods.buy(id).send({from: account, value: price})
      .on('receipt', async (r) => {
        update(dispatch)
        window.alert(`Congratulations, you've received NFT Hack with ID: ${id}\nAddress: ${Contract.networks[netId].address}`)
      })
      .on('error',(error) => {
        console.error(error)
    ...
    console.log('Error, buy NFT Hack', e)
  }
